...
contract FlatTreasury is Ownable {
    ...
    address public sFLAT;
    uint public sFLATQueue; // Delays change to sFLAT address
    ...
    /**
        @notice allow approved address to deposit an asset for FLAT
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint FLAT needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( FLAT ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn FLAT for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IFLATERC20( FLAT ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sFLAT ).balanceOf( msg.sender ); // Can only borrow against sFLAT held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with FLAT
        @param _amount uint
     */
    function repayDebtWithFLAT( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IFLATERC20( FLAT ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, FLAT, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( FLAT ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( FLAT ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns FLAT valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match FLAT decimals
            value_ = _amount.mul( 10 ** IERC20( FLAT ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SFLAT ) { // 9
            sFLATQueue = uint32(block.timestamp).add32( secondsNeededForQueue );
        } else return false;
